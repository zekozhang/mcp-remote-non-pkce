...
Use `mcp-remote-non-pkce` or `mcp-remote-client-non-pkce` instead of the standard binaries. The non-PKCE mode **requires** `--static-oauth-client-info` to be provided:
...
**Complete Example for Claude Desktop:**
...
For more details on standard (PKCE) OAuth support and other features, see the sections below.
